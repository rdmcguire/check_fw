...
class firewall_tool:
        ...
        if 'client' in self.config.keys() and 'targets' in self.config['client'].keys():
            ...
            if len(self.config['client']['targets']) > 0:
                for target in self.config['client']['targets']:
                    self.print_log('Scanning host '+target,bold=True)
                    for port in self.config['client']['targets'][target]['ports']:
                        self.print_log('Testing port '+str(port),indent=2,newline=False)
    ...
    def server_port(self, port):
        ...
        if not sock.connect_ex(addr):
            self.print_log('Port {} is listening'.format(str(addr)),sev='good',indent=3)
        else:
            self.print_log('Port {} not ready, starting...'.format(str(addr)),indent=3)
    ...
    def print_log(self,msg,indent=1,sev='info',bold=False,no_pre=False,dt=False,newline=True):
		...
		# Timestamp
        if dt:
            pre = colored(datetime.now(),"white","on_grey",attrs=["bold"]) + pre
		# Msg Style
        style = []
        if bold:
            style = ['bold']
